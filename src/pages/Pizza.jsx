...
export default function Pizza() {
  ...
  const { datos } = useContextApi();
  const { id } = useParams();

  useEffect(() => {
    datos.map((ingredient) => setPizzaIngredients(ingredient.ingredients));
    const detailedPizzaSelected = (id) => {
      const pizzaSelected = datos.filter((item) => {
        ...
        <div className="d-flex justify-content-center mt-5" key={i}>
          ...
          <div className="mx-5">
            ...
            <div className="d-flex justify-content-between mx-5">
              ...
              <Link to={"/cart"} className="btn btn-danger ms-1">
                Añadir 🛒
              </Link>
            </div>
          </div>
